...
def get_texts(corpus):
	...
	'''
	texts = []
	text_ids = pd.unique(corpus.TEXTID)
	for t in text_ids:
		text = corpus[corpus.TEXTID==t]
		texts.append(text)
	return texts
...
def split_text(text):
	'''
	LM can't take too many tokens at once. So we split the text into 4 subtexts.
	'''
	split_interv = text.WNUM.max() // 4
	text1 = text[text.WNUM <= split_interv]
	text2 = text[(text.WNUM > split_interv) & (text.WNUM <= (split_interv*2))]
	text3 = text[(text.WNUM > (split_interv*2)) & (text.WNUM <= (split_interv*3))]
	text4 = text[text.WNUM > split_interv*3]
...
def main(RT_corpus, output_file, tokenizer, model, if_test=True):
	...
	for text in texts:
		...
		for idx, subtext in enumerate(subtexts):
			...
			subtext['SUBTEXT_WNUM'] = subtext.groupby('SUBTEXTID').cumcount()
			df = pd.concat([df, subtext])
		print('Completed: Text {}'.format(text.TEXTID.iloc[0]))
