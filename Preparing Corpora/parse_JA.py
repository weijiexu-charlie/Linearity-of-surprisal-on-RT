...
def get_texts(corpus):
	...
	'''
	texts = []
	text_ids = pd.unique(corpus['article'])
	for t in text_ids:
		text = corpus[corpus['article']==t]
		texts.append(text)
	return texts
...
def add_text_logp(text, tokenizer, model, token_space):
	'''
	...
	'''
	tokens_corpus = text['surface'].tolist()
	if token_space:
...
def main(RT_corpus, output_file, tokenizer, model, if_test=True):
	...
	'''
	japanese = pd.read_csv(RT_corpus)
	texts = get_texts(japanese)
	if if_test:
		texts = texts[:2]   # Only use the first two texts if testing
	df = pd.DataFrame()
	for text in texts:
		add_text_logp(text, tokenizer, model, token_space=False)
