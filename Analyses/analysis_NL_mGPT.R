library(Rmisc)
library(tidyverse)
library(stringr)
library(scales)
library(grid)
library(ggpubr)
library(MASS)
library(lme4)
library(stats)
library(modelr)
library(plotrix)
library(mgcv)
library(hexbin)
library(formattable)

rm(list=ls())

dutch <- read.csv("preproc_data/dutch_mGPT.csv")


#########################################################
################### Adapted Spillover ###################
#########################################################

# ---------------------- Step 1 ----------------------
### Model selection; Determine spillover using length and frequency as controls
### Maximal converging random effects for both models to be compared
### Result: Justified up to spillover 3

# Spillover 1
nl.s0 = lmer(RT ~ logfreq.s + wlen.s + 
               (1|subj),
             data=na.omit(dutch),
             REML=FALSE) 
summary(nl.s0)    
nl.s1 = lmer(RT ~ logfreq.s + logfreq1.s + wlen.s + wlen1.s + 
               (1|subj), 
             data=na.omit(dutch),
             REML=FALSE) 
summary(nl.s1)
anova(nl.s0, nl.s1)  # Significant (Spillover 1 is needed)

# Spillover 2
nl.s11 = lmer(RT ~ logfreq.s + logfreq1.s + wlen.s + wlen1.s + 
                (1|subj), 
              data=na.omit(dutch),
              REML=FALSE) 
summary(nl.s11)
nl.s2 = lmer(RT ~ logfreq.s + logfreq1.s + logfreq2.s + wlen.s + wlen1.s + wlen2.s + 
               (1|subj), 
             data=na.omit(dutch),
             REML=FALSE)
summary(nl.s2)
anova(nl.s11, nl.s2)  # Significant (Spillover 2 is needed)

# Spillover 3:
nl.s22 = lmer(RT ~ logfreq.s + logfreq1.s + logfreq2.s + wlen.s + wlen1.s + wlen2.s +
              (1|subj), 
              data=na.omit(dutch),
              REML=FALSE) 
summary(nl.s22)
nl.s3 = lmer(RT ~ logfreq.s + logfreq1.s + logfreq2.s + logfreq3.s + wlen.s + wlen1.s + wlen2.s + wlen3.s + 
              (1|subj), 
             data=na.omit(dutch),
             REML=FALSE)
summary(nl.s3)
anova(nl.s22, nl.s3)  # Significant (Spillover 3 is needed)


# ---------------------- Step 2 ----------------------
### Examine Linearity
### Result: Significant quadratic effect, but sublinear

# Null
poly0 <- lmer(RT ~ logfreq.s + logfreq1.s + logfreq2.s + logfreq3.s + wlen.s + wlen1.s + wlen2.s + wlen3.s +
                (1|subj), 
              data=na.omit(dutch),
              REML=FALSE)
summary(poly0) 

# Linear
poly1 <- lmer(RT ~ logp.s + logp1.s + logp2.s + logp3.s + logfreq.s + logfreq1.s + logfreq2.s + logfreq3.s + wlen.s + wlen1.s + wlen2.s + wlen3.s +
                (1|subj), 
              data=na.omit(dutch),
              REML=FALSE)
summary(poly1)
anova(poly0, poly1)   # Significant linear

# Nonlinear
poly2 <- lmer(RT ~ logp.s + logp1.s + logp2.s + logp3.s + logfreq.s + logfreq1.s + logfreq2.s + logfreq3.s + wlen.s + wlen1.s + wlen2.s + wlen3.s + I(logp.s^2) + I(logp1.s^2) + I(logp2.s^2) + I(logp3.s^2) +
                (1|subj), 
              data=na.omit(dutch),
              REML=FALSE)
summary(poly2)   
anova(poly1, poly2)   # Significant quadratic, but sublinear
 

###########################################################
################ Fixed Spillover (2 words) ################
###########################################################

### Examine Linearity
### Result: Non-significant quadratic term

# Null
poly0.spill2 <- lmer(RT ~ logfreq.s + logfreq1.s + logfreq2.s + wlen.s + wlen1.s + wlen2.s +
                (1|subj), 
              data=na.omit(dutch),
              REML=FALSE)
summary(poly0.spill2) 

# Linear
poly1.spill2 <- lmer(RT ~ logp.s + logp1.s + logp2.s + logfreq.s + logfreq1.s + logfreq2.s + wlen.s + wlen1.s + wlen2.s +
                (1|subj), 
              data=na.omit(dutch),
              REML=FALSE)
summary(poly1.spill2)
anova(poly0.spill2, poly1.spill2)  # Significant linear

# Nonlinear
poly2.spill2 <- lmer(RT ~ logp.s + logp1.s + logp2.s + logfreq.s + logfreq1.s + logfreq2.s + wlen.s + wlen1.s + wlen2.s + I(logp.s^2) + I(logp1.s^2) + I(logp2.s^2) +
                (1|subj), 
              data=na.omit(dutch),
              REML=FALSE)
summary(poly2.spill2)   
anova(poly1.spill2, poly2.spill2)  # ns, p=0.1007

